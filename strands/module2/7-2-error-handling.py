...
1. Hooks con Retry: Para errores transitorios
2. Try except en herramientas: para servicios externos
3. Conversation managers para context overflow
3. Try/except en el loop para sesion interactiva
4. Fallbacks para degradacion elegante: si falla una herramienta, usar otra o responder con lo que sepa
"""
...
# Manejo de errores - Estrageia 1: Retry
class RetryPlugin(Plugin):
    name = "retry-plugin"

    @hook
    def retry_failed_tools(self, event: AfterToolCallEvent) -> None:
        if isinstance(event.result, Exception):
            print(f"⚠️ La herramienta {event.tool_use['name']} falló. Reintentando...")
            # Aquí podrías implementar lógica para reintentar la herramienta
            if retries < 3:
                retries += 1
                event.retry = True  # Indica que se debe reintentar la herramienta
                # event.agent.call_tool(event.tool_use['name'], event.tool_use['args'])
...
# Manejo de errores - Estrategia 2: reintar con backoff exponencial
@tool
def consultar_api(endpoint: str) -> str:
    """
    Consulta una API externa para obtener información adicional.

    Args:
        endpoint: La URL del endpoint de la API a consultar.
    """
    for attempt in range(3):  # Intentar hasta 3 veces
        print(f"Intentando consultar la API (intento {attempt + 1})...")
        try:
            response = requests.get(endpoint, timeout=5)
            # response.raise_for_status()  # Lanza un error si la respuesta no es 200
            # return response.json()
            return response.text  # Para simplificar, devolvemos el texto de la respuesta
        except requests.exceptions.RequestException as e:
            print(f"Error al consultar la API: {e}")
            time.sleep(2 ** attempt)  # Exponential backoff: 1s, 2s ,4s
        return "Error al consultar la API. Por favor, inténtalo de nuevo más tarde."


# Manejo de errores - Estrategia 4: Degradacion elegante
@tool
def buscar_docs(query: str) -> str:
    """
    Busca documentación en la base de conocimiento local.

    Args:
        query: La consulta de búsqueda.
    """
    # Aquí podrías implementar la lógica para buscar en la documentación local
    try:
        # Simulación de búsqueda en documentación
        return hacer_busqueda(query) # esto va a dar error porque no existe la funcion hacer_busqueda
    except Exception as e:
        return "No se pudo acceder a la docs. Respondo con lo que sepa."
...
def main(agente):
    conversacion_actual = []
    print("¿En qué te puedo ayudar? (escribe 'salir' para terminar)\n")

    while True:
        user_input = input("Usuario: ")

        # Manejo de errores - Estrategia 3: Captura de excepciones en el loop principal
        try:
            if user_input.lower() in ["salir", "exit", "quit"]:
                # Generar ticket antes de salir si hubo conversación
                if conversacion_actual:
                    print("\n📝 Generando ticket de soporte...\n")
                    ticket = crear_ticket(agente, "\n".join(conversacion_actual))
...
if __name__ == "__main__":
    ...
    if consulta == "2":
        session_id = input("Ingresa tu ID de sesion: ")
    else:
        session_id = str(uuid.uuid4())[:8]
        print(f"Tu ID de sesion es {session_id}")
        print("Guardalo para retomar despues")
    ...
    agente = Agent(
        ...
        - Cuando uses documentacion de AWS, citá la fuente y el link
        """,
        # tools=[buscar_solucion, consultar_api, aws_docs_client],
        # tools=[buscar_solucion, consultar_api],
        tools=[consultar_api, buscar_docs],
        model=bedrock_Model,
        plugins=[LoggingPlugin(), RetryPlugin()],

        session_manager = session_manager,
        conversation_manager=SummarizingConversationManager(
            summary_ratio=0.3, # resumir el 30% cuando contexto se llene
            preserve_recent_messages=10 # siempre retener ultimos 10
        )
    )
    main(agente)
